...
describe("Users", () => {
  ...
  describe("UsersDeleteAvatar", () => {
    ...
    beforeAll(async () => {
      ...
      // Explicitly set this avatar as active (previous tests may have set a different one)
      await setActiveAvatarHandler(
        buildAuthEvent(userTestUser, {
          method: "PUT",
          body: { avatarId: activeAvatarId },
        }),
      );
    ...
    test("deletes non-active avatar", async () => {
      // Second avatar should be deletable since first one stays active
      const event = buildAuthEvent(userTestUser, {
        method: "DELETE",
        pathParameters: {
          avatarId: deletableAvatarId, // Second one is not active
        },
      });
    ...
    test("returns 409 when trying to delete active avatar", async () => {
      const event = buildAuthEvent(userTestUser, {
        method: "DELETE",
        pathParameters: {
          avatarId: activeAvatarId, // First avatar stays active
        },
      });
