...
describe("AuthRegister", () => {
  // Track userIds created during tests for cleanup
  ...
  test("returns 409 when username already exists", async () => {
    ...
    const firstResponse = await registerHandler(event1);
    if (firstResponse.statusCode === 201) {
      createdUserIds.push(parseBody<{ userId: string }>(firstResponse).userId);
    }
